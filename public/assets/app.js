(function () {
  ...
  function renderAnalysis() {
    ...
    const relationTypeLabel = (type) => type === "formal" ? t("relationFormal") : type === "watch" ? t("relationWatch") : t("relationInsufficient");
    $("#analysis-relations").innerHTML = analysis.relation_diagnostics.map((item) => `<tr class="${item.relation_type !== "formal" ? "evidence-row-weak" : ""}"><td>${escapeHtml(relationName(item))}<span class="table-evidence-note">${escapeHtml(relationTypeLabel(item.relation_type))}</span></td><td>${escapeHtml(item.conclusion)}${item.can_enter_summary ? "" : `<span class="table-evidence-note">${t("insufficientEvidence")}</span>`}</td><td>${escapeHtml(listText(item.evidence))}</td><td>${escapeHtml(item.risk_note)}</td><td>${escapeHtml(confidenceText(item.confidence))}</td></tr>`).join("");
    const divergenceCard = (item) => `<article class="divergence-card"><div class="divergence-head"><h4>${escapeHtml(item.title)}</h4><span class="severity ${item.divergence_type === "confirmed_divergence" ? "high" : ""}">${escapeHtml(item.divergence_type === "confirmed_divergence" ? t("confirmedDivergence") : t("potentialDivergence"))}</span></div><p>${escapeHtml(item.interpretation)}</p><div class="divergence-watch">${t("watchlist")} · ${escapeHtml(listText(item.what_to_watch_next))}</div></article>`;
    const confirmed = (analysis.confirmed_divergences || []).slice(0, 3);
    const potential = (analysis.potential_divergences || []).slice(0, 3);
    $("#analysis-divergences").innerHTML = confirmed.length || potential.length ? `${confirmed.length ? `<h4>${t("confirmedDivergence")}</h4>${confirmed.map(divergenceCard).join("")}` : ""}${potential.length ? `<h4>${t("potentialDivergence")}</h4>${potential.map(divergenceCard).join("")}` : ""}` : `<div class="empty-analysis">${t("noDivergence")}</div>`;
